<template>
  <div class="max-w-2xl mx-auto p-6 space-y-8 pb-24">
    ...
    <div v-if="reports.length > 0" class="space-y-4">
      <h2 class="text-xs font-bold text-dark-500 uppercase tracking-widest">Active Tickets</h2>
      <div v-for="report in reports" :key="report._id" class="glass-card hover:border-primary-500/30 transition-all">
        ...
        <div class="mt-4 pt-4 border-t border-dark-100 flex justify-end">
          <button @click="viewReport(report)" class="text-primary-600 text-xs font-bold uppercase tracking-wider hover:text-primary-700">
            View Thread
          </button>
        </div>
      </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-dark-100 shadow-sm relative overflow-hidden">
      ...
      <form @submit.prevent="submitReport" class="space-y-4">
        <div>
          <label class="block text-xs font-bold text-dark-400 uppercase tracking-widest mb-2">Subject</label>
        ...
        <div v-if="orders.length > 0">
          <label class="block text-xs font-bold text-dark-400 uppercase tracking-widest mb-2">Related Order (Optional)</label>
        ...
        <div>
          <label class="block text-xs font-bold text-dark-400 uppercase tracking-widest mb-2">Details</label>
          <textarea 
        ...
        <button 
          type="submit" 
          class="btn-primary w-full py-4 text-sm uppercase tracking-widest font-bold mt-4"
