  ...
  <div class="space-y-8 animate-fade-in pb-32 selection:bg-parentPrimary/10 selection:text-parentPrimary">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
      <div class="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar scroll-smooth">
        <button
          ...
          class="px-5 py-2.5 rounded-xl text-[10px] font-bold transition-all uppercase tracking-widest whitespace-nowrap border"
    ...
    <div v-else-if="filteredOrders.length === 0" class="px-2">
       <div class="bg-white rounded-[2rem] text-center p-16 border border-dashed border-gray-100 animate-fade-in">
          ...
          <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-8">Start your first Errand on campus</p>
          <NuxtLink to="/vendors" class="inline-flex px-8 py-4 bg-gray-900 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-parentPrimary transition-all">Start Shopping</NuxtLink>
       </div>
    </div>
      ...
      <div class="space-y-3">
        ...
        >
          <div class="flex items-center gap-3 flex-1 min-w-0">
            <div class="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center text-xl shadow-inner border border-white group-hover:bg-parentPrimary/10 transition-colors shrink-0">
              {{ statusEmoji(order.status) }}
            </div>
            <div class="min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-bold text-gray-900 text-[13px] tracking-tight truncate flex-1 md:flex-none">#{{ order.orderNumber }}</h3>
                <span :class="getStatusBadge(order.status)" class="sm:hidden text-[7px] font-black uppercase tracking-widest px-2 py-1 rounded-md border shrink-0">
                  {{ formatStatus(order.status) }}
                </span>
              </div>
              <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest truncate">{{ order.vendor?.storeName || 'Custom Errand' }}</p>
            </div>
          </div>

          <div class="text-right px-2 shrink-0">
            <p class="text-[11px] font-bold text-gray-900 leading-none mb-1">₦{{ order.total?.toLocaleString() }}</p>
            <p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest">{{ timeAgo(order.createdAt) }}</p>
          </div>

          <div class="shrink-0 flex items-center gap-3">
            <span :class="getStatusBadge(order.status)" class="hidden sm:block text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border">
              {{ formatStatus(order.status) }}
            </span>
    ...
    <SideDrawer :isOpen="!!selectedOrder" @close="selectedOrder = null">
      <template v-if="selectedOrder">
        <div class="flex flex-col items-center justify-center py-10 border-b border-gray-50 text-center px-6">
           ...
           <span :class="getStatusBadge(selectedOrder.status)" class="text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl border bg-white">
          ...
          <div class="bg-white rounded-2xl p-6 space-y-6 border border-gray-100 shadow-sm">
             <div class="flex items-center gap-4">
                ...
                <div class="space-y-0.5">
                   <p class="text-[8px] font-bold uppercase tracking-widest text-gray-400">Vendor</p>
             ...
             <div class="pt-6 border-t border-gray-50 flex justify-between items-end">
                 <div class="space-y-0.5">
                    <p class="text-[8px] font-bold text-parentPrimary uppercase tracking-widest">Total Amount</p>
                    <p class="text-2xl font-bold text-gray-900 tracking-tight">₦{{ selectedOrder.total?.toLocaleString() }}</p>
                 </div>
                 <div class="text-[8px] font-bold text-gray-300 uppercase tracking-widest">NGN</div>
             </div>
          </div>

          <!-- Actions -->
          <div class="space-y-3">
            <h4 class="text-[8px] font-black uppercase tracking-[0.2em] text-gray-300 ml-2 mb-2">Actions</h4>
            
            <button 
               @click="navigateTo(`/dashboard/orders/${selectedOrder._id}`)" 
               class="w-full py-4 bg-gray-900 text-white rounded-xl text-[10px] font-extrabold uppercase tracking-widest shadow-lg shadow-gray-900/20 hover:bg-parentPrimary transition-all flex items-center justify-center gap-2 group"
            >
            ...
            <button
               ...
               class="w-full py-4 bg-white text-gray-900 rounded-xl text-[10px] font-extrabold uppercase tracking-widest border border-gray-100 shadow-sm hover:bg-gray-50 transition-all flex items-center justify-center gap-2"
            >
               Order Again
            </button>
            ...
            <button
               ...
               class="w-full py-3.5 bg-rose-50 text-rose-500 rounded-xl text-[9px] font-bold uppercase tracking-widest border border-rose-100 hover:bg-rose-500 hover:text-white transition-all"
            >
               Cancel Errand
            </button>
          </div>
