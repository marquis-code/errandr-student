  ...
  <div class="space-y-12 pb-32 animate-fade-in selection:bg-parentPrimary/10 selection:text-parentPrimary">
    ...
    <section class="relative">
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div class="space-y-1">
          ...
          <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Your campus companion, delivered fast.</p>
        </div>
        <div class="hidden md:flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-xl shadow-sm">
           ...
           <span class="text-emerald-700 text-[9px] font-bold uppercase tracking-wider">Vendors Online</span>
        </div>
      </div>
    ...
    <section class="space-y-8">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <NuxtLink to="/meal-planner" class="group relative bg-gray-900 rounded-2xl p-6 overflow-hidden border-b-2 border-gray-800 shadow-sm transition-all hover:scale-[1.01] active:scale-[0.98]">
           ...
           <div class="relative z-10 flex items-center gap-4">
              ...
              <div class="space-y-0.5">
                 <h4 class="text-base font-black text-white uppercase tracking-tighter">Meal Planner</h4>
                 <p class="text-[8px] font-bold text-gray-500 uppercase tracking-[0.2em]">Budget & Track</p>
              </div>
           </div>
        </NuxtLink>
        ...
        <NuxtLink to="/errands/custom" class="group relative bg-white rounded-2xl p-6 overflow-hidden border border-gray-50 shadow-sm hover:shadow-md transition-all hover:scale-[1.01] active:scale-[0.98]">
           ...
           <div class="relative z-10 flex items-center gap-4">
              ...
              <div class="space-y-0.5">
                 <h4 class="text-base font-black text-gray-900 transition-colors uppercase tracking-tighter">Order Anything</h4>
                 <p class="text-[8px] font-bold text-parentPrimary uppercase tracking-[0.2em]">Custom Errand</p>
              </div>
           </div>
        </NuxtLink>
      </div>
      ...
      <div class="flex items-center gap-3 overflow-x-auto pb-4 hide-scrollbar -mx-4 px-4 lg:mx-0 lg:px-0">
        <button v-for="cat in categories" :key="cat.name" class="flex-shrink-0 flex items-center gap-2.5 px-4 py-3 bg-white rounded-xl border border-gray-100 shadow-sm hover:border-parentPrimary hover:scale-105 transition-all group min-w-[120px]">
          ...
          <span class="text-[9px] font-bold uppercase tracking-wider text-gray-600 group-hover:text-gray-900">{{ cat.name }}</span>
        </button>
      </div>
    </section>
    ...
    <section class="space-y-8">
      <div class="flex items-end justify-between px-2">
        <div class="space-y-1">
          ...
          <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Top rated partners on campus</p>
        </div>
        <NuxtLink to="/vendors" class="text-parentPrimary text-[10px] font-bold uppercase tracking-[0.2em] hover:tracking-[0.3em] transition-all">All Vendors &rarr;</NuxtLink>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="flex items-center gap-4 px-2 pb-2">
             <div class="relative">
                <img :src="vendor.logo || 'https://images.unsplash.com/photo-1594212699903-ec8a3eca50f5?w=200&h=200&fit=crop'" class="w-12 h-12 rounded-xl object-cover border-2 border-white shadow-md transition-transform duration-700 group-hover:scale-105 bg-white" />
                <div class="absolute -right-0.5 -bottom-0.5 w-3 h-3 bg-emerald-500 rounded-full border-2 border-white shadow-sm" />
             </div>
             <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-[9px] font-bold text-emerald-600 uppercase tracking-widest">Open</span>
                </div>
             </div>
          </div>
        </NuxtLink>
    ...
    <section class="relative bg-gray-900 rounded-2xl p-8 lg:p-10 overflow-hidden group border-b-[4px] border-gray-800 shadow-lg">
      ...
      <div class="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
         <div class="space-y-4">
            <div class="inline-flex items-center gap-2 px-2 py-0.5 bg-white/5 border border-white/10 rounded-md text-white text-[7px] font-bold uppercase tracking-[0.2em]">
               Student Program
            </div>
            <h2 class="text-2xl md:text-3xl font-black text-white leading-tight tracking-tight">
               Earn as an <br /><span class="text-parentPrimary">Errandr Provider.</span>
            </h2>
            ...
            <a href="http://errandr.localhost:3000/auth/register" class="inline-flex items-center gap-3 px-8 py-3.5 bg-parentPrimary text-white rounded-xl font-bold text-[10px] shadow-lg shadow-parentPrimary/20 hover:scale-105 transition-all">
    ...
    <section v-if="onlineVendors.length > 3" class="space-y-8">
      <div class="flex items-center gap-4 px-2">
         <h2 class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Fresh Arrivals</h2>
         <div class="flex-1 h-px bg-gray-100" />
      </div>
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
         <NuxtLink v-for="vendor in onlineVendors.slice(3, 8)" :key="vendor._id" :to="`/vendors/${vendor._id}`" class="group">
            <div class="aspect-square rounded-[2rem] bg-white border border-gray-100 shadow-sm p-6 flex flex-col items-center justify-center text-center hover:shadow-md hover:border-parentPrimary transition-all duration-500 relative overflow-hidden">
              ...
              <p class="text-[9px] font-bold text-parentPrimary uppercase tracking-widest mt-2 opacity-0 group-hover:opacity-100 transition-all duration-300 relative z-10">Visit &rarr;</p>
            </div>
         </NuxtLink>
      </div>
    </section>
