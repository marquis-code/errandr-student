  ...
  <div class="min-h-screen bg-white selection:bg-parentPrimary/10 selection:text-parentPrimary font-sans">
    ...
    <div v-if="cartStore.isEmpty.value" class="min-h-screen flex flex-col">
      ...
      <div class="flex-1 flex flex-col items-center justify-center p-6 max-w-lg mx-auto text-center">
        ...
        <NuxtLink to="/vendors" class="px-8 py-4 bg-gray-900 text-white rounded-2xl font-black text-xs tracking-wider hover:bg-parentPrimary transition-all active:scale-95 flex items-center gap-2 shadow-lg">
      ...
      <div class="max-w-5xl mx-auto w-full px-4 md:px-6 pb-12">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-lg font-black text-gray-900 tracking-tight">Campus Favorites 🔥</h3>
          <NuxtLink to="/vendors" class="text-xs font-black text-parentPrimary hover:underline px-3 py-1.5 rounded-lg bg-parentPrimary/5">See all</NuxtLink>
        </div>
      ...
      <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-gray-50">
        <div class="max-w-[1200px] mx-auto flex items-center justify-between px-4 md:px-6 py-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-base font-black text-gray-900 tracking-tight leading-none">Checkout</h1>
              <p class="text-[10px] font-bold text-gray-400 mt-0.5">{{ cartStore.itemCount }} items</p>
            </div>
          </div>
          <button 
            @click="showOrderBreakdown = true"
            class="flex items-center gap-2 px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-xs font-black text-gray-900 hover:bg-gray-100 transition-all lg:hidden active:scale-95"
          >
          ...
          <div class="flex-1 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              ...
              <div class="p-5 space-y-4">
                ...
                <div v-if="deliveryOption === 'use_an_errander'" class="animate-fade-in">
                  <label class="text-[10px] font-black text-gray-400 tracking-wider block mb-2 pl-1">Hostel / Campus Location</label>
                  <textarea
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div class="flex items-center gap-3 px-5 py-4 bg-gray-50/50 border-b border-gray-100">
                ...
                <h3 class="text-sm font-black text-gray-900 tracking-tight">Extras</h3>
              </div>
              <div class="p-4 space-y-1">
                <label class="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer group">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="text-xs font-black text-gray-900 leading-none mb-0.5">Mystery Box</p>
                ...
                <label class="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer group">
                  <div class="flex items-center gap-3">
                    <span class="text-lg">🏢</span>
                    <div>
                      <p class="text-xs font-black text-gray-900 leading-none mb-0.5">Dorm Delivery</p>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div class="flex items-center gap-3 px-5 py-4 bg-gray-50/50 border-b border-gray-100">
                ...
                <div>
                  <h3 class="text-sm font-black text-gray-900 tracking-tight">Payment Method</h3>
              ...
              <div class="p-4">
                <div class="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    class="p-4 rounded-2xl border-2 transition-all text-left flex flex-col gap-3 active:scale-[0.98]"
                  >
                    <div class="w-10 h-5 overflow-hidden">
                      <img src="~/assets/img/paystack.png" class="h-full object-contain" alt="Paystack" />
                    </div>
                    <div>
                      <p class="text-xs font-black text-gray-900 leading-none mb-0.5">Pay with Card</p>
                  ...
                  >
                    ...
                    <div>
                      <p class="text-xs font-black text-gray-900 leading-none mb-0.5">My Wallet</p>
                      <p :class="balance < finalTotal ? 'text-rose-500' : 'text-emerald-600'" class="text-xs font-black">
                ...
                <div v-if="paymentMethod === 'wallet' && balance < finalTotal" class="mt-4 p-4 bg-rose-50 border border-rose-100 rounded-2xl animate-fade-in text-center">
                  ...
                  <div class="flex flex-col gap-2">
                    <input 
                      ...
                      class="w-full px-4 py-3 bg-white border border-rose-200 rounded-xl text-sm font-black text-gray-900 outline-none focus:border-parentPrimary transition-all placeholder:text-gray-300"
                    />
                    <button 
                      @click="startTopup"
                      :disabled="loadingPayment || topupAmount <= 0"
                      class="w-full py-3 bg-gray-900 text-white rounded-xl text-xs font-black hover:bg-parentPrimary transition-all disabled:opacity-50"
                    >
            ...
            <div class="hidden lg:block bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div class="flex items-center justify-between px-5 py-4 bg-gray-50/50 border-b border-gray-100">
                <div class="flex items-center gap-3">
                  ...
                  <h3 class="text-sm font-black text-gray-900 tracking-tight">Order Items</h3>
                </div>
                <button @click="goBack" class="text-[10px] font-black text-parentPrimary hover:underline">Edit Items</button>
              </div>
                ...
                <div v-for="vendorId in cartStore.allVendorIds.value" :key="vendorId" class="space-y-4">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        <h4 class="text-xs font-black text-gray-900 tracking-tight">{{ toTitleCase(vendorsMetadata[vendorId]?.storeName || 'Loading...') }}</h4>
                  ...
                  <div v-for="(pack, pIndex) in cartStore.getVendorStats(vendorId).packs" :key="pack.id" class="pl-4 border-l-2 border-gray-50 space-y-3">
                    <span class="text-[9px] font-black bg-gray-900 text-white px-2.5 py-1 rounded-md">{{ pack.name || `Pack ${pIndex + 1}` }}</span>
                    <div v-for="(item, iIndex) in pack.items" :key="item.productId + iIndex" class="flex items-center gap-3">
                      ...
                      <div class="flex-1 min-w-0">
                        <h5 class="text-xs font-black text-gray-900 truncate">{{ toTitleCase(item.name) }}</h5>
                        <p class="text-xs font-bold text-parentPrimary">₦{{ item.price?.toLocaleString() }} <span class="text-gray-400">×{{ item.quantity }}</span></p>
                      </div>
                      <span class="text-xs font-black text-gray-900">₦{{ (item.price * item.quantity).toLocaleString() }}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="sticky top-[65px] space-y-5">
              <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                <div class="px-5 py-4 bg-gray-50/50 border-b border-gray-100">
                  <h3 class="text-xs font-black text-gray-900 tracking-tight">Order Summary</h3>
                </div>
                <div class="p-5 space-y-3">
                  <div class="flex justify-between items-center text-xs font-bold text-gray-500">
                    <span>Subtotal</span>
                    <span class="text-gray-900 font-black">₦{{ (groupOrder ? groupSubtotal : cartStore.subtotal.value)?.toLocaleString() }}</span>
                  </div>
                  <div v-if="computedTotalDeliveryFee > 0" class="flex justify-between items-center text-xs font-bold text-gray-500">
                    <span>Delivery</span>
                    <span class="text-gray-900 font-black">₦{{ computedTotalDeliveryFee.toLocaleString() }}</span>
                  </div>
                  <div class="flex justify-between items-center text-xs font-bold text-gray-500">
                    <span>Fees & Packaging</span>
                    <span class="text-gray-900 font-black">₦{{ (computedTotalPackagingFee + computedTotalServiceFee).toLocaleString() }}</span>
                  </div>
                  <div v-if="computedPaystackFee > 0" class="flex justify-between items-center text-xs font-bold text-gray-500">
                    <span>Processing Fee</span>
                    <span class="text-gray-900 font-black">₦{{ computedPaystackFee.toLocaleString() }}</span>
                  </div>
                </div>
                <div class="px-5 pb-5 pt-3 border-t border-dashed border-gray-100 space-y-5">
                  <div class="flex justify-between items-end">
                    <div>
                      <p class="text-[10px] font-black text-gray-400 tracking-wider mb-1">Total</p>
                  ...
                  <button 
                    ...
                    class="w-full py-4 bg-parentPrimary text-white rounded-2xl text-xs font-black tracking-wider hover:bg-parentPrimary/90 transition-all disabled:opacity-40 flex items-center justify-center gap-2 shadow-xl shadow-parentPrimary/20 active:scale-[0.98]"
                  >
      ...
      <div class="fixed bottom-6 left-4 right-4 z-40 lg:hidden">
        <div class="bg-gray-900 rounded-2xl px-5 py-4 text-white flex items-center justify-between shadow-2xl shadow-black/30">
          <div>
            ...
            <h3 class="text-xl font-black text-white tracking-tighter leading-none">₦{{ finalTotal.toLocaleString() }}</h3>
          </div>
          <button 
            @click="startPayment"
            :disabled="placing || (paymentMethod === 'wallet' && balance < finalTotal)"
            class="px-6 py-3.5 bg-parentPrimary text-white rounded-xl text-xs font-black tracking-wider hover:scale-105 active:scale-95 transition-all disabled:opacity-30 flex items-center gap-2 shadow-lg shadow-parentPrimary/30"
          >
      ...
      >
        <div v-if="showOrderBreakdown" class="fixed inset-x-0 bottom-0 z-[101] bg-white rounded-t-[2rem] max-h-[85vh] overflow-hidden flex flex-col lg:hidden shadow-2xl">
          <div class="sticky top-0 bg-white pt-3 pb-2 px-5 border-b border-gray-50 z-10 rounded-t-[2rem]">
            ...
            <div class="flex items-center justify-between">
              <div>
                <h2 class="text-lg font-black text-gray-900 tracking-tight">Review Items</h2>
          ...
          <div class="flex-1 overflow-y-auto px-5 py-4 space-y-6 pb-8 scrollbar-hide">
            <div v-for="vendorId in cartStore.allVendorIds.value" :key="'mb-' + vendorId" class="space-y-4">
              <div class="flex items-center gap-2.5">
                ...
                <h4 class="text-xs font-black text-gray-900">{{ toTitleCase(vendorsMetadata[vendorId]?.storeName || '') }}</h4>
              </div>
              ...
              <div v-for="(pack, pIndex) in cartStore.getVendorStats(vendorId).packs" :key="'mb-pack-' + pack.id" class="pl-3 border-l-2 border-gray-50 space-y-3">
                <div class="flex items-center justify-between">
                  <span class="text-[9px] font-black bg-gray-900 text-white px-2.5 py-1 rounded-md">{{ pack.name || `Pack ${pIndex + 1}` }}</span>
                  <button @click="cartStore.removePack(vendorId, pack.id)" class="text-[10px] font-black text-rose-400 hover:text-rose-500">Remove</button>
                </div>
          ...
          <div class="px-5 py-4 border-t border-gray-100 bg-white">
            <button @click="showOrderBreakdown = false" class="w-full py-3.5 bg-gray-900 text-white rounded-2xl text-xs font-black tracking-wider active:scale-[0.98] transition-transform">
              Done Reviewing
            </button>
          </div>
        </div>
      ...
      >
        <div v-if="showTopupModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="showTopupModal = false">
          <div class="bg-white rounded-[2rem] w-full max-w-sm p-8 text-center animate-zoom-in">
            ...
            <h2 class="text-xl font-black text-gray-900 mb-2 tracking-tight">Fund Wallet</h2>
            <p class="text-xs font-medium text-gray-400 mb-6">Enter the amount to add. You'll be redirected to Paystack.</p>
            <div class="space-y-4">
              <div class="text-left">
                <p class="text-[10px] font-black text-gray-400 tracking-wider mb-2 pl-1">Amount (NGN)</p>
                <input 
                  v-model="topupAmount" 
                  type="number"
                  placeholder="e.g. 5000"
                  class="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:border-emerald-500/20 rounded-xl text-sm font-black text-gray-900 outline-none transition-all"
                />
              </div>
              <button 
                @click="startTopup" 
                :disabled="loadingPayment || topupAmount <= 0"
                class="w-full py-4 bg-gray-900 text-white rounded-xl text-xs font-black hover:bg-emerald-500 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <Loader2 v-if="loadingPayment" class="w-4 h-4 animate-spin" />
                <span>Fund Wallet Now</span>
              </button>
            </div>
            <button @click="showTopupModal = false" class="mt-5 text-xs font-black text-gray-300 hover:text-gray-900 transition-colors">Maybe Later</button>
          </div>
        </div>
      </Transition>
    </Teleport>
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="placing" class="fixed inset-0 z-[200] bg-gray-900/95 backdrop-blur-xl flex flex-col items-center justify-center text-center p-6 text-white">
          ...
          <h2 class="text-2xl font-black mb-2 tracking-tight">Finalizing order...</h2>
